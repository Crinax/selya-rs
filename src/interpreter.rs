...
pub fn interprete(memory: &mut Box<Memory>, tokens: Vec<Token>) -> Result<(), InterpreterError> {
    ...
    for token in tokens {
        if next_should_be_number {
            ...
            if let Token::Number(value) = token {
                ...
                match token_before.unwrap() {
                    Token::Add => if let Err(value) = memory.add(value) {
                        return Err(InterpreterError::MemErr(value));
                    },
                    Token::AddModulo => { memory.add_modulo(value); }
                    _ => {
                        return Err(InterpreterError::UsingBinaryAsUnary);
                    }
                };
        ...
        match token {
            Token::Number(value) => { memory.write(value); }
            Token::ShiftLeft => if let Err(value) = memory.shift_left() {
                return Err(InterpreterError::MemErr(value));
            },
            Token::ShiftRight => if let Err(value) = memory.shift_right() {
                return Err(InterpreterError::MemErr(value));
            },
            Token::ShiftWordLeft => { memory.shift_word_left(); },
            Token::ShiftWordRight => { memory.shift_word_right(); },
            Token::Add => {
